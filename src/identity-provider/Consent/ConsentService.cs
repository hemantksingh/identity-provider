...
namespace identity
{
    public class ConsentService
    {
        ...
        public async Task<ConsentViewModel> BuildViewModelAsync(string returnUrl, ConsentInputModel model = null)
        {
            ...
            if (request != null)
            {
                ...
                if (client != null)
                {
                    ...
                    {
                        LoggerExtensions.LogError(_logger, "No scopes matching: {0}", request.ScopesRequested.Aggregate((x, y) => x + ", " + y));
                    }
                }
                else
                {
            ...
            Client client, Resources resources)
        {
            ...
            vm.IdentityScopes = resources.IdentityResources.Select(x => CreateScopeViewModel(x, Enumerable.Contains<string>(vm.ScopesConsented, x.Name) || model == null)).ToArray();
            vm.ResourceScopes = resources.ApiResources.SelectMany(x => x.Scopes).Select(x => CreateScopeViewModel(x, Enumerable.Contains<string>(vm.ScopesConsented, x.Name) || model == null)).ToArray();
            if (ConsentOptions.EnableOfflineAccess && resources.OfflineAccess)
            {
                vm.ResourceScopes = Enumerable.Union(vm.ResourceScopes, new ScopeViewModel[] {
                    GetOfflineAccessScope(Enumerable.Contains<string>(vm.ScopesConsented, IdentityServer4.IdentityServerConstants.StandardScopes.OfflineAccess) || model == null)
                });
            }
